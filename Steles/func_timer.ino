...
void updateTimer() {
	...
	if (micr < ABSOLUTETIME) { // If the micros-value has wrapped around its finite counting-space to be less than the last absolute-time position...
		ELAPSED = (4294967295 - ABSOLUTETIME) + micr; // Put the wrapped-around microseconds into the elapsed-time value
	} else { // Else, if the micros-value is greater than or equal to last loop's absolute-time position...
		ELAPSED += micr - ABSOLUTETIME; // Add the difference between the current time and the previous time to the elapsed-time value
	}
