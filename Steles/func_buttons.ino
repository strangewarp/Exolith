	...
	if ((CTRL & B00100001) == B00100001) { // If this is a PAGE SEQS OFF command, which requires an additional seq-key strike to activate...
		...
		if (cue | nums) { // If there is a CUE or any NUMBERS present...
			for (byte i = pstart; i < pstart + 24; i++) { // For each seq on the active page...
				CMD[i] = (nums << 4) | 1; // Cue an OFF command for the global cue-point that corresponds to the given NUMBERS
			}
		...
		if (CMD[seq] & B00000010) { // If the sequence already contains a cued ON value...
			...
			word chunk = (STATS[seq] * 96) >> 3; // Get the number of ticks in one of the seq's slice-chunks
			POS[seq] = 32768 | ( // Set the sequence to playing...
				(chunk * nums) + ( // With a base position equal to the slice-numbers being held...
					( // Added to an additional tick-offset inside of the slice itself, calculated as follows:
						(POS[seq] & 32768) ? // Depending on whether the sequence is already playing...
						POS[seq] : // Either take the sequence's current tick...
