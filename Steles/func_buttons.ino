...
void parsePlayPress(byte col, byte row) {
	...
	} else if (ctrl == B00010001) { // If SCATTER UNSET is held...
	...
	if (ctrl && (ctrl != B00000101)) { // If this was a non-BPM control-command...
...
void assignKey(byte col, byte row) {
	...
	if (col == 0) { // If the keystroke is in the leftmost column...
		...
		if (RECORDMODE) { // If RECORD-MODE is active...
			...
			if (ctrl == B00000101) { // And a BPM command is newly-held...
...
void unassignKey(byte col, byte oldcmds) {
	if (RECORDMODE) { // If RECORD-MODE is active...
		...
		if (
			((BUTTONS & B00111111) == B00000101) // If a BPM command is active...
