...
void unassignKey(byte col) {
	if (RECORDMODE) { // If RECORD-MODE is active...
		if (col == 0) { // If this up-keystroke was in the leftmost column...
			TO_UPDATE |= 253; // Flag the top LED-row, and bottom 6 LED-rows, for updating
		} else { // Else, if this up-keystroke was within the note-buttons...
			if (REPEAT && (!BUTTONS)) { // If REPEAT is enabled, but the last held-button was just released...
				if (ARPLAST) { ARPLAST--; } // Reduce the position of the last arpeggio-note to compensate for key-release
				ARPDIR = 1; // Reset arpeggio-direction to 1 (upwards, default)
				ARPBUTTON = 0; // Reset tracking of the last arpeggio-button
			}
		}
	}
}
