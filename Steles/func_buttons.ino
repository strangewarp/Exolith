...
void parseRecPress(byte col, byte row) {
	...
	if (CTRL == B00000110) { // If a SAVE command is held...
		saveSong(key); // Save the current tempfile into a savefile slot
	} else if (CTRL == B00011000) { // If a LOAD command is held...
		loadSong(key); // Load a save-slot's contents into its tempfile, and start using that file
	} else if (CTRL == B00000010) { // If a BASE-NOTE/OCTAVE/VELO/HUMANIZE PANEL button is held...
		if (row == 0) { // If a BASE-NOTE button was pressed...
			BASENOTE = min(12, BASENOTE ^ (1 << col)); // Modify the BASENOTE value
		} else if (row == 1) { // If an OCTAVE button was pressed...
			OCTAVE = min(10, OCTAVE ^ (1 << col)); // Modify the OCTAVE value
		} else if (row <= 3) { // If a VELOCITY button was pressed...
			VELOCITY = min(127, max(1, VELOCITY ^ (1 << (((row - 2) * 4) + col)))); // Modify the VELOCITY value
		} else { // If a HUMANIZE button was pressed...
			HUMANIZE = min(127, max(1, HUMANIZE ^ (1 << (((row - 4) * 4) + col)))); // Modify the HUMANIZE value
		}
	} else if (CTRL == B00000100) { // If a CHAN/TIME-QUANTIZE/DUR/SEQ-SIZE PANEL button is held...
		if (row == 0) { // If a CHANNEL button was pressed...
			CHAN ^= 1 << col; // Modify the CHANNEL value
		} else if (row == 1) { // If a TIME-QUANTIZE button was pressed...
			QUANTIZE ^= 1 << col; // Modify the TIME-QUANTIZE value
		} else if (row <= 3) { // If a DURATION button was pressed...
			DURATION ^= 1 << (((row - 2) * 4) + col); // Modify the DURATION value
		} else { // If a SEQ-SIZE button was pressed...
			SEQ_SIZE[RECORDSEQ] ^= 1 << (((row - 4) * 4) + col); // Modify the currently-recording-seq's size
		}
	} else if (CTRL == B00001000) { // If a CLOCK-MODE/CHAN-LISTENS/BPM PANEL button is held...
		if (row == 0) { // If a CLOCK-MODE button was pressed...
			CLOCKMODE = col >> 1; // Modify the CLOCK-MODE value (0 or 1)
		} else if (row <= 3) { // If any of the 3 lanes' worth of CHAN-LISTEN buttons were pressed...
			CHAN_LISTEN[row - 1] ^= 1 << col; // Modify the CHAN-LISTEN slot that corresponds to the given row
		} else { // If a BPM button was pressed...
			BPM = max(15, BPM ^ (1 << (((row - 4) * 4) + col))); // Modify the BPM value (bounds: 15 to 255)
		}
	} else if (CTRL == B00010000) { // If a SWITCH RECORDING SEQUENCE button is held...
