...
void updateGUI() {
	...
	if (TO_UPDATE & 1) { // If the first row is flagged for a GUI update...
		if (!(ctrl && RECORDMODE)) { // If no control-buttons are held, and we aren't in recording-mode...
			lc.setRow(0, 0, 128 >> ((CUR16 >> 4) - 1)); // Display the global-tick position
		} else { // Else, if-and-only-if control-buttons are held AND we are in recording-mode...
	...
	if (TO_UPDATE & 2) { // If the second row is flagged for a GUI update...
		lc.setRow(0, 1, (128 >> PAGE) | ctrl); // Display current PAGE and CTRL-commands
	}
