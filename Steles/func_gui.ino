...
void updateFirstRow(byte ctrl) {
	if (RECORDMODE) { // If this is RECORD-MODE...
		...
		} else if (ctrl == B00100010) { // Else, if SWING AMOUNT is held...
			if (SAMOUNT > 64) { // If SWING AMOUNT is positive...
				byte bm = SAMOUNT & 63; // Get a trimmed version of the SAMOUNT value
				bm = ((bm & 1) << 7) // Flip that trimmed SAMOUNT value's endianness
					| ((bm & 2) << 5) // ^
					| ((bm & 4) << 3) | ((bm & 32) >> 3) // ^
					| ((bm & 8) << 1) | ((bm & 16) >> 1); // ^
				sendRow(0, 255 ^ bm); // Send the flipped trimmed value to look like it's leaning right
			} else if (SAMOUNT < 64) { // Else, if SWING AMOUNT is negative...
				sendRow(0, 255 ^ (64 - SAMOUNT)); // Send the SAMOUNT value to look like it's leaning left
			} else { // Else, if SWING AMOUNT is directly centered...
				sendRow(0, 255); // Fill the row
			}
		} else if (ctrl == B00100001) { // Else, if SWITCH RECORDING-SEQUENCE is held...
			sendRow(0, RECORDSEQ); // Show the currently-active seq
		} else if (ctrl == B00010010) { // Else, if QUANTIZE-RESET is held...
			sendRow(0, QRESET); // Display QRESET value
		} else if (ctrl == B00010001) { // Else, if SWING GRANULARITY is held...
			sendRow(0, 255 >> ((4 - SGRAN) * 2)); // Fill the top row, corresponding to current SWING GRANULARITY
