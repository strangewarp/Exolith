...
void updateFirstRow(byte ctrl) {
	if (RECORDMODE) { // If this is RECORD-MODE...
		if (ctrl == B00100000) { // If ARM RECORDING is pressed...
			...
			sendVirtualCharRow(SAMOUNT, 64); // Send a virtual char-value to the top LED-row
			...
			sendRow(0, 1 << (SGRAN - 1)); // Fill the top row, corresponding to current SWING GRANULARITY
		} else if (ctrl == B00001001) { // Else, if CLOCK-MASTER is pressed...
			sendRow(0, CLOCKLEAD * 255); // Light up row if CLOCKLEAD mode is active
		} else if (ctrl == B00000111) { // Else, if GRID-CONFIG is pressed...
			sendRow(0, GRIDCONFIG); // Display GRIDCONFIG value
		} else if (ctrl == B00111000) { // Else, if ARP-MODE is pressed...
			sendRow(0, ~(15 << (ARPMODE * 4))); // Display ARP-MODE value (0 = left side on; 1 = right side on; 2 = both sides on)
		} else if (ctrl == B00110100) { // Else, if ARP-REFRESH is pressed...
	...
	for (byte i = 2; i < 8; i++) { // For each of the bottom 6 GUI rows...
		if (TO_UPDATE & (1 << i)) { // If the row is flagged for an update...
			...
			if (ctrl == B00000101) { // If a BPM command is held...
				row |= pgm_read_byte_near(GLYPHS + 136 + i); // Display a line from the BPM glyph
