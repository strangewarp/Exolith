...
void updateFirstRow(byte ctrl) {
	if (RECORDMODE) { // If this is RECORD-MODE...
		if (ctrl == B00100000) { // If ARM RECORDING is pressed...
			...
			sendRow(0, (STATS[RECORDSEQ] & 31) + 1); // Display current record-seq's SIZE value
      ...
      || (ctrl == B00011010)
      || (ctrl == B00001101)
    ) { // Else, if any AUTOCMD-related chord is held...
      sendRow(0, AUTOCURSOR); // Display the AUTOCMD-CURSOR position
    } else if (ctrl == B00000111) { // Else, if ACTIVE ON LOAD is pressed...
      sendRow(0, 255 * (!!(STATS[RECORDSEQ] & 64))); // Display the current RECORDSEQ's ACTIVE ON LOAD value (0 = empty row; 64 = full row)
		} else { // Else...
...
void updatePlayBottomRows(byte ctrl) {
	...
	for (byte i = 2; i < 8; i++) { // For each of the bottom 6 GUI rows...
		if (TO_UPDATE & (1 << i)) { // If the row is flagged for an update...
			...
			if (ctrl == B00000101) { // If a BPM command is held...
				row |= pgm_read_byte_near(GLYPHS + 148 + i); // Display a line from the BPM glyph
			} else if (ctrl == B00000011) { // Else, if a SHIFT POSITION command is held...
				row |= pgm_read_byte_near(GLYPHS + 106 + i); // Display a line from the SHIFT POSITION glyph
