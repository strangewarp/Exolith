...
void updateFirstRow(byte ctrl) {
	if (RECORDMODE) { // If this is RECORD-MODE...
		...
		} else if (ctrl == B00101000) { // Else, if CHAN is pressed...
			lc.setRow(0, 0, CHAN); // Display CHAN value
		} else if (ctrl == B00100100) { // Else, if CONTROL-CHANGE is pressed...
			lc.setRow(0, 0, (!!(CHAN & 16)) * 255); // Light up row if the CONTROL-CHANGE flag is active
		} else if (ctrl == B00010010) { // Else, if PROGRAM-CHANGE is pressed...
			lc.setRow(0, 0, (!!(CHAN & 32)) * 255); // Light up row if the PROGRAM-CHANGE flag is active
